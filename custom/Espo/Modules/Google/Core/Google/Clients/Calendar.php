...
class Calendar extends Google
{
    ...
    public function insertEvent($calendarId, $event)
    {
        ...
        try {
            return $this->request($url, json_encode($event), 'POST', 'application/json');
        } catch (Exception $e) {
            if ($e instanceof RequestError && $e->getCode() === 409) {
                $errors = $e->getErrorData()->errors ?? null;
                $error = $errors[0] ?? null;

                if (
                    $error instanceof stdClass &&
                    ($error->reason ?? null) === 'duplicate'
                ) {
            ...
            $GLOBALS['log']->error('GoogleCalendarERROR: ' . $e->getMessage());
            $GLOBALS['log']->debug('GoogleCalendarERROR: Params: ' . print_r($event, true));
    ...
    public function updateEvent($calendarId, $eventId, $modification)
    {
        ...
        try {
            return $this->request($url, json_encode($modification), 'PUT', 'application/json');
        } catch (Exception $e) {
            $GLOBALS['log']->error('GoogleCalendarERROR: ' . $e->getMessage());
