 ...
 *
 * License ID: 7c9d34c02c060e385617e86b0bb9a65b
 ************************************************************************************/
...
use Espo\Modules\Google\Core\Google\Exceptions\DuplicateError;
...
class Event extends Base
{
    ...
    public function insertIntoGoogle($espoEvent)
    {
        ...
        if (!empty($googleEvent)) {
            ...
            try {
                ...
                $response = $this->obtainEventUid($this->getCalendarId(), $espoEvent);
            ...
            $responseId = is_array($response) && isset($response['id']) ?
                $response['id'] : null;
            ...
            if ($responseId) {
                ...
                if ($toUpdate) {
                    $this->getClient()->updateEvent($this->getCalendarId(), $responseId, $googleEvent);
                }

                return true;
            }

            // @todo Count failed attempts and mark the event to exclude from sync somehow.
            //    Store in event relation?
        }
     ...
     * @return ?array<string, mixed>
     */
    private function obtainEventUid(string $calendarId, array $event): ?array
    {
        $uid = $event['uid'] ?? null;
        ...
        $result = $this->getClient()->getEventList($calendarId, ['iCalUID' => $uid]);
